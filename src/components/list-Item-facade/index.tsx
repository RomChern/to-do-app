...
import styles from "./list-item-facade.module.css"
...
export const ListItemFacade = ({ item, onStartEditing }: TProps) => {
    const toDoChangeTools = useContext(ToDoChangeToolsContext);

    const handleClick = () => {
        toDoChangeTools?.completeToDo(item.id)
    }

    const handleDeleteClick = () => {
        toDoChangeTools?.deleteToDo(item.id)
    }

    return <>
        <button className={styles.todoItemsLeft} onClick={handleClick}>
            <svg
                clipRule="evenodd"
                fillRule="evenodd"
        ...
        <div className={styles.todoItemsRight}>
